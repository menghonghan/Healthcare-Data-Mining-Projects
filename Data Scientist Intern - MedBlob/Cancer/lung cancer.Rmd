...
colnames(table_final1) = c("SEQN","Gender","Mexican_American","Other_Hispanic","NonHispanic_White","NonHispanic_Black",  "NonHispanic_Asian", "RaceOther Race - Including Multi-Racial","Close_Relative_Heart_Attack","Close_Relative_Asthma","Close_Relative_Diabetes","Overweight","Lack_Exercise","High_Salt","Hight_Fat_Calories","Have_Chronic_bronchitis","Smoked_at_least_100_cigarettes_in_life","Smoked_a_cigar_even_one_time","Ever_used_an_ecigarette","Ever_used_smokeless_tobacco","Ever_had_pain_or_discomfort_in_chest","Breathe_paint_fumes_past_48hours","Breathe_diesel_fumes_past_48hours","Breathe_fingernail_polish_fumes_past_48hours","Age","Exposure_to_secondhand_smoke","Urinary_arsenic_total_(ug/L)","Chromium_(nmol/L)","AVG_Betacarotene","y")
...
demo1 <-as.data.frame(cbind(SEQN=DEMO$SEQN,Gender=DEMO$RIAGENDR,Age=DEMO$RIDAGEYR,Race))
demo1$Gender <- case_when(demo1$Gender==1~1, demo1$Gender==2~0)
demo1$Gender <- as.factor(demo1$Gender)
nrow(demo1)
```
...
med1 <- medic %>% select(-"MCQ230A") %>% drop_na()
...
download.file("https://wwwn.cdc.gov/Nchs/Nhanes/2017-2018/SMQ_J.XPT", tf, mode="wb")
# Create Data Frame From Temporary File & Keep Variables Of Interest                                              
smoke<- read.xport(tf)[,c("SEQN","SMQ020","SMQ890","SMQ900","SMQ910")]
smoke$SMQ020<- case_when(smoke$SMQ020==1 ~ 1, smoke$SMQ020==2 ~ 0)
smoke$SMQ020<- as.factor(smoke$SMQ020)
...
```{r}
# Download NHANES 2015-2016 Exposure to second-hand smoke Data 
download.file("https://wwwn.cdc.gov/Nchs/Nhanes/2017-2018/SMQFAM_J.XPT", tf, mode="wb")
# Create Data Frame From Temporary File & Keep Variables Of Interest                                              
secondsmoke<- read.xport(tf)[,c("SEQN","SMD460")]
secondsmoke1<-secondsmoke[!(secondsmoke$SMD460==777 | secondsmoke$SMD460==999),]
...
```{r}
# Download NHANES 2015-2016 Chest Pain  Data 
download.file("https://wwwn.cdc.gov/Nchs/Nhanes/2017-2018/CDQ_J.XPT", tf, mode="wb")
# Create Data Frame From Temporary File & Keep Variables Of Interest                                              
chestpain<- read.xport(tf)[,c("SEQN","CDQ001")]
chestpain$CDQ001<- case_when(chestpain$CDQ001==1 ~ 1, chestpain$CDQ001==2 ~ 0)
chestpain$CDQ001<- as.factor(chestpain$CDQ001)
chestpain1<- na.omit(chestpain)
```
...
```{r}
# Download NHANES 2015-2016 Volatile Toxicant  Data 
download.file("https://wwwn.cdc.gov/Nchs/Nhanes/2017-2018/VTQ_I.XPT", tf, mode="wb")
# Create Data Frame From Temporary File & Keep Variables Of Interest                                              
Toxicant<- read.xport(tf)[,c("SEQN","VTQ281A","VTQ281C","VTQ281E")]
